...
def process_file(raw_path: Path):
    ...
    if ".raw" not in name:
        print(f"[SKIP] Not a raw file: {name}")
        return

    base = name.split(".raw", 1)[0]

    clean_path = raw_path.with_name(f"{base}.clean.v4_2.md")
    meta_path  = raw_path.with_name(f"{base}.clean.v4_2.meta.json")
    ...
    # step 1.1: remove duplicate metadata
